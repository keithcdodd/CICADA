...
% Determine whether the cleaned IMAGE itself is a CICADA product.
% Use only the filename, not the full filepath, because parent folders may
% themselves contain the word "CICADA".
if contains(cleaned_file_info.name, 'CICADA', 'IgnoreCase', true)
    cicada = 1;
else
    cicada = 0;
end

% Set up CICADA-specific information only when the cleaned file is CICADA.
data_signal_mask = '';
...
if cicada == 1
    ...
    if contains(cleaned_file_info.name, 'manual', 'IgnoreCase', true)
        cicada_type = 'manual';
...
if cicada == 1
    ...
    % If a data_signal_mask exists, grab it.
    data_signal_mask_path = ...
        [task_dir, '/ic_', cicada_type, ...
        '_selection/funcmask_CICADA_', ...
        cicada_type, '_signal_constrained.nii.gz'];

    if isfile(data_signal_mask_path)

        data_signal_mask = data_signal_mask_path;

    else

        data_signal_mask_path = ...
            [task_dir, '/funcmask_CICADA_', ...
            cicada_type, '_signal_constrained.nii.gz'];

        if isfile(data_signal_mask_path)
            data_signal_mask = data_signal_mask_path;
        else
            fprintf(['Could not find a data_signal_mask at ', ...
                data_signal_mask_path, '\n'])
            data_signal_mask = '';
        end
    ...
    % Expanded CICADA support products.
    support_products = ...
        cicada_locate_support_products(task_dir, cicada_type);


    % GM probability mask used for CICADA signal/GM overlap.
    gm_mni_prob_info = ...
        dir([task_dir, '/region_masks/GM_prob.nii.gz']);
    ...
    gm_mni_prob = fullfile( ...
        gm_mni_prob_info.folder, ...
        gm_mni_prob_info.name);

    gm_mni_thresh_array = ...
        niftiread(gm_mni_prob) > 0.67;


    % CICADA DecisionVariables contain component-selection information.
    ic_select_dir = ...
        [task_dir, '/ic_', cicada_type, '_selection'];

    data_vals_info = ...
        dir([ic_select_dir, '/DecisionVariables*.mat']);

    if numel(data_vals_info) ~= 1
        fprintf('Cannot uniquely locate CICADA DecisionVariables file.\n')
        return;
    end

    data_vals = fullfile( ...
        data_vals_info.folder, ...
        data_vals_info.name);
...
if numel(cleaned_file_niftiinfo.ImageSize) >= 4
    numvolumes = cleaned_file_niftiinfo.ImageSize(4);
else
    numvolumes = 1;
end
...
% Ratio of GM temporal variance to non-GM temporal variance.
% Calculate this separately for each dataset.
cleaned_GM_div_NotGM_mean_var = ...
    cleaned_GM_mean_var ./ cleaned_NotGM_mean_var;

compare_GM_div_NotGM_mean_var = ...
    compare_GM_mean_var ./ compare_NotGM_mean_var;

orig_GM_div_NotGM_mean_var = ...
    orig_GM_mean_var ./ orig_NotGM_mean_var;
...
if cicada == 1
    ...
    % We want good GM coverage by signal without signal spreading excessively
    % outside GM. Use the Jaccard overlap coefficient (intersection / union).
    gm_signal_intersection = sum(gm_signal_overlap_array(:));
    
    gm_or_signal_array = ...
        (gm_mni_thresh_array + signal_mask_array) > 0;
    
    gm_signal_union = sum(gm_or_signal_array(:));
    
    if gm_signal_union > 0
    
        gm_signal_jaccard_array = ...
            gm_signal_intersection ./ gm_signal_union;
    
    else
    
        gm_signal_jaccard_array = NaN;
    ...
    cleaned_qc_array = [{cleaned_file, cleaned_file_info.name, sub_id, ses_id, task_name, adjusted, cleaned_DS, meanRMS, ...
        medFD, meanFD, perc_FD_above_thresh, Any_FD_above_thresh, medDVARS, gm_signal_jaccard_array, gm_signal_proportion_array, signal_gm_proportion_array, ...
        Results.num_ICs_kept, Results.num_ICs_total, percent_ICs_kept, Results.percent_variance_kept, ...
        numvolumes, DOF_estimate_if_standard_bp, ...
        ...
        cleaned_GM_mean_var, cleaned_CSF_mean_var, cleaned_WMCSF_mean_var, cleaned_Outbrain_mean_var, cleaned_Edge_mean_var, cleaned_Suscept_mean_var, cleaned_GM_div_NotGM_mean_var}, num2cell(Results.compare_cleaning.After'), ...
    ...
    qc_labels = ['image_path', 'image_names', 'subject', 'session', 'task', 'manually_adjusted', 'denoising_success_score', 'meanRMS', ...
        'median_FD', 'mean_FD', 'Percent_FD_gt_point2mm','AnyFD_gt_5mm', 'median_DVARS', 'gm_signal_jaccard', 'gm_coverage_by_signal', 'signal_overlap_with_gm', ...
    ...
    compare_qc_array = {compare_file, compare_file_info.name, sub_id, ses_id, task_name, compare_DS, meanRMS, ...
        ...
        numvolumes, compare_NotGM_NotGM_mean_abs, compare_GM_GM_mean_abs, compare_DVARS_GM_mean_abs, ...
        ...
        compare_CSF_mean_var, compare_WMCSF_mean_var, compare_Outbrain_mean_var, compare_Edge_mean_var, compare_Suscept_mean_var, compare_GM_div_NotGM_mean_var};
    ...
    orig_qc_array = {orig_file, orig_file_info.name, sub_id, ses_id, task_name, orig_DS, meanRMS, ...
        medFD, meanFD, perc_FD_above_thresh, Any_FD_above_thresh, medDVARS, ...
        numvolumes, orig_NotGM_NotGM_mean_abs, orig_GM_GM_mean_abs, orig_DVARS_GM_mean_abs, ...
        ...
        orig_CSF_mean_var, orig_WMCSF_mean_var, orig_Outbrain_mean_var, orig_Edge_mean_var, orig_Suscept_mean_var, orig_GM_div_NotGM_mean_var};
    ...
    medFD, ...
    meanFD, ...
    perc_FD_above_thresh, ...
    Any_FD_above_thresh, ...
    medDVARS, ...
    numvolumes, ...
    cleaned_NotGM_NotGM_mean_abs, ...
    cleaned_GM_GM_mean_abs, ...
    cleaned_DVARS_GM_mean_abs, ...
    cleaned_FD_GM_mean_abs, ...
    cleaned_CSF_CSF_mean_abs, ...
    cleaned_WMCSF_WMCSF_mean_abs, ...
    cleaned_Outbrain_Outbrain_mean_abs, ...
    cleaned_Edge_Edge_mean_abs, ...
    cleaned_Suscept_Suscept_mean_abs, ...
    cleaned_NotGM_mean_var, ...
    cleaned_GM_mean_var, ...
    cleaned_CSF_mean_var, ...
    cleaned_WMCSF_mean_var, ...
    cleaned_Outbrain_mean_var, ...
    cleaned_Edge_mean_var, ...
    cleaned_Suscept_mean_var, ...
    cleaned_GM_div_NotGM_mean_var};
