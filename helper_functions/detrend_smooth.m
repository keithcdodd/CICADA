...
if size(funcmask_data) ~= size(file_orig_data)
...
if smoothing_kernel ~= 0
    ...
    for idx2 = 1:size(file_orig_data,4)
        file_data(:,:,:, idx2) = imgaussfilt3(file_detrended(:,:,:, idx2), round(smoothing_kernel ./ mm_div)); % convert kernel from mm to voxel
    end
    file_data(funcmask == 0) = 0; % mask it with funcmask
    % write to data dir and relabel cleaned file:
    niftiwrite(file_data, [output_dir, '/s_', file_name], file_orig_data_info, "Compressed", true)
    cleaned_file = [data_dir, '/s_', file_name, '.nii.gz']; % update cleaned_file

else
    file_data = file_detrended; % not smoothed, only detrended if smoothing kernel was set at 0
    file_data(funcmask == 0) = 0; % mask it with funcmask
    % write to data dir and relabel cleaned file:
    niftiwrite(file_data, [data_dir, '/', file_name], file_orig_data_info, "Compressed", true)
    cleaned_file = [data_dir, '/', file_name, '.nii.gz']; % update cleaned_file
end
