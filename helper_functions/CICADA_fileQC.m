...
[denoised_GM, denoised_GM_mean, denoised_GM_var] = getData_sameregion(denoised_file_data, funcmask, GM_prob_file);
...
[denoised_NotGM, ~, denoised_NotGM_var] = getData_sameregion(denoised_file_data, funcmask, NotGM_prob_file);
[denoised_Edge, ~, denoised_Edge_var] = getData_sameregion(denoised_file_data, funcmask, Edge_prob_file);
[denoised_Outbrain, ~, denoised_Outbrain_var] = getData_sameregion(denoised_file_data, funcmask, Outbrain_prob_file);
[denoised_WMCSF, ~, denoised_WMCSF_var] = getData_sameregion(denoised_file_data, funcmask, WMCSF_prob_file);
[denoised_CSF, ~, denoised_WMCSF_var] = getData_sameregion(denoised_file_data, funcmask, CSF_prob_file);
[denoised_Suscept, ~, denoised_Suscept_var] = getData_sameregion(denoised_file_data, funcmask, Suscept_prob_file);

% consider with orig file as well, can then compare "autocorrelations"
[orig_GM, ~, orig_GM_var] = getData_sameregion(orig_file_data, funcmask, GM_prob_file);
[orig_NotGM, ~, orig_NotGM_var] = getData_sameregion(orig_file_data, funcmask, NotGM_prob_file);
[orig_Edge, ~, orig_Edge_var] = getData_sameregion(orig_file_data, funcmask, Edge_prob_file);
[orig_Outbrain, ~, orig_Outbrain_var] = getData_sameregion(orig_file_data, funcmask, Outbrain_prob_file);
[orig_WMCSF, ~, orig_WMCSF_var] = getData_sameregion(orig_file_data, funcmask, WMCSF_prob_file);
[orig_CSF, ~, orig_CSF_var] = getData_sameregion(orig_file_data, funcmask, CSF_prob_file);
[orig_Suscept, ~, orig_Suscept_var] = getData_sameregion(orig_file_data, funcmask, Suscept_prob_file);
...
FD_GM_corr = createHistConf1DData(denoised_GM, confounds_fd, GM_randperm, 10000); % make it same sampling as others
...
GM_GM_var = createVarHistData(denoised_GM, orig_GM, GM_randperm, perms);
Edge_Edge_var = createVarHistData(denoised_Edge, orig_Edge, Edge_randperm, perms);
Outbrain_Outbrain_var = createVarHistData(denoised_Outbrain, orig_Outbrain, Outbrain_randperm, perms);
WMCSF_WMCSF_var = createVarHistData(denoised_WMCSF, orig_WMCSF, WMCSF_randperm, perms);
CSF_CSF_var = createVarHistData(denoised_CSF, orig_CSF, CSF_randperm, perms);
NotGM_NotGM_var = createVarHistData(denoised_NotGM, orig_NotGM, NotGM_randperm, perms);
Suscept_Suscept_var = createVarHistData(denoised_Suscept, orig_Suscept, Suscept_randperm, perms);
...
function compare_orig_var_comp = createVarHistData(compare_region_var, orig_region_var, region_randperm, perms)
% Variance should be the least reduced in GM regions compared to orig
perms = min([size(compare_region_var,1), size(orig_region_var, 1), perms]); % should be the same sizes
compare_orig_var_comp = compare_region_var(region_randperm(1:perms),:)' ./ orig_region_var(region_randperm(1:perms),:)'; % divide denoised variance by orig variance

% make into a column, get rid of Inf values, and make size of perms
compare_orig_var_comp = compare_orig_var_comp(~isinf(compare_orig_var_comp)); % gets rid of inf and also makes it into a column

% make it size of perms
compare_orig_var_randperms = randperm(length(compare_orig_var_comp));
nums = min(length(compare_orig_var_comp), 10000);
compare_orig_var_comp = compare_orig_var_comp(compare_orig_var_randperms(1:nums)); % make it 10000 max, because otherwise too much big data
