function [Edge_Edge_corr, FD_GM_corr, DVARS_GM_corr, Outbrain_Outbrain_corr, WMCSF_WMCSF_corr, CSF_CSF_corr, NotGM_NotGM_corr, GM_GM_corr, Suscept_Suscept_corr, GM_mean, mean_var_table] = CICADA_fileQC(cleaned_dir, denoised_file, orig_file)
...
mean_var_array = [denoised_GM_mean_var, denoised_NotGM_mean_var, denoised_Edge_mean_var, ...
...
mean_var_table = array2table(mean_var_array, 'VariableNames', mean_var_labels);

end

%%
function [GM, region, Global_Signal, GM_Signal_mean, GM_mean_var, region_mean_var] = getData(funcfile_data, funcmask, GM_prob_file, region_prob_file)
% read in the niftis
...
GM_mean_var = mean(var(GM, 0, 2), "omitnan"); % variance of difference in time for GM)
