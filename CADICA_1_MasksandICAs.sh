...
do
  ...
  ### Edgemask: any functional outside of anatomical inbrain, include susceptibility
  fslmaths ${funcmask} -sub "${anatmaskdir}/Anatprob_resam.nii.gz" "${anatmaskdir}/Edgeprob_resam.nii.gz"
  fslmaths "${anatmaskdir}/Edgeprob_resam.nii.gz" -thr 0.5 -mul "${funcmask}" -bin "${anatmaskdir}/EdgeOnly_prop_final.nii.gz"
  fslmaths "${anatmaskdir}/Edgeprob_resam.nii.gz" -max "${anatmaskdir}/Susceptibilitymask_prop_final.nii.gz" -thr 0.5 -mul "${funcmask}" -bin "${anatmaskdir}/Edge_prop_final.nii.gz"

  ## Calculate relative proportion between GM, WM, and CSF, while removing Edge from GM and WM (but can keep for CSF because they are supposed to share space!)
  ...
  # fslmaths "${anatmaskdir}/GMprob_resam.nii.gz" -div "${anatmaskdir}/summedprobs_resam.nii.gz" -sub "${anatmaskdir}/Edgemask_prop_final.nii.gz" -thr 0.5 -mul "${funcmask}" "${anatmaskdir}/GM_prop_final.nii.gz"
  # fslmaths "${anatmaskdir}/WMprob_resam.nii.gz" -div "${anatmaskdir}/summedprobs_resam.nii.gz" -sub "${anatmaskdir}/Edgemask_prop_final.nii.gz" -thr 0.5  -mul "${funcmask}" "${anatmaskdir}/WM_prop_final.nii.gz"
  # fslmaths "${anatmaskdir}/CSFprob_resam.nii.gz" -div "${anatmaskdir}/summedprobs_resam.nii.gz" -sub "${anatmaskdir}/Edgemask_prop_final.nii.gz" -thr 0.5 -mul "${funcmask}" "${anatmaskdir}/CSF_prop_final.nii.gz"

  # We could calculate a max mask of all the probabilities to eventually mask whole brain
  # fslmaths "${anatmaskdir}/Edgeprob_resam.nii.gz" -max "${anatmaskdir}/GMprob_resam.nii.gz" -max "${anatmaskdir}/WMprob_resam.nii.gz" -max "${anatmaskdir}/CSFprob_resam.nii.gz" -mul ${funcmask} "${anatmaskdir}/maxprobanat.nii.gz"
  # fslmaths "${anatmaskdir}/Edgeprob_resam.nii.gz" -div "${anatmaskdir}/maxprobanat.nii.gz" -thr 0.99 -mul "${anatmaskdir}/maxprobanat.nii.gz" "${anatmaskdir}/Edge_prop_final.nii.gz"
  # fslmaths "${anatmaskdir}/GMprob_resam.nii.gz" -div "${anatmaskdir}/maxprobanat.nii.gz" -thr 0.99 -mul "${anatmaskdir}/maxprobanat.nii.gz" "${anatmaskdir}/GM_prop_final.nii.gz"
  # fslmaths "${anatmaskdir}/WMprob_resam.nii.gz" -div "${anatmaskdir}/maxprobanat.nii.gz" -thr 0.99 -mul "${anatmaskdir}/maxprobanat.nii.gz" "${anatmaskdir}/WM_prop_final.nii.gz"
  # fslmaths "${anatmaskdir}/CSFprob_resam.nii.gz" -div "${anatmaskdir}/maxprobanat.nii.gz" -thr 0.99 -mul "${anatmaskdir}/maxprobanat.nii.gz" "${anatmaskdir}/CSF_prop_final.nii.gz"

  fslmaths "${anatmaskdir}/GMprob_resam.nii.gz" -sub "${anatmaskdir}/Susceptibilityprob_resam.nii.gz" -thr 0.5 -mul "${funcmask}" -bin "${anatmaskdir}/GM_prop_final.nii.gz"
  fslmaths "${anatmaskdir}/WMprob_resam.nii.gz" -sub "${anatmaskdir}/Susceptibilityprob_resam.nii.gz" -thr 0.5 -mul "${funcmask}" -bin "${anatmaskdir}/WM_prop_final.nii.gz"
  fslmaths "${anatmaskdir}/CSFprob_resam.nii.gz" -sub "${anatmaskdir}/Susceptibilityprob_resam.nii.gz" -thr 0.5 -mul "${funcmask}" -bin "${anatmaskdir}/CSF_prop_final.nii.gz"

  ### WMCSF boundary (internal veins), GMCSF boundary (internal arteries), Outbrain, and Susceptibilitymask_prop_final
  # We can multiply WM and CSF and rescale it to get WMCSF boundary for Subependymal
  fslmaths "${anatmaskdir}/WMprob_resam.nii.gz" -mul "${anatmaskdir}/CSFprob_resam.nii.gz" "${anatmaskdir}/WMCSF_boundary.nii.gz"
  fslstats "${anatmaskdir}/WMCSF_boundary.nii.gz" -R > ${anatmaskdir}/range.txt
  minimumvalue=$(awk '{print $1}' ${anatmaskdir}/range.txt)
  maximumvalue=$(awk '{print $2}' ${anatmaskdir}/range.txt)
  fslmaths "${anatmaskdir}/WMCSF_boundary.nii.gz" -div "${maximumvalue}" -thr 0.5 -bin "${anatmaskdir}/WMCSF_boundary_prop_final.nii.gz"
  ...
  # melodic --in="${chosen_funcfile}" --outdir=${melfol} --mask="${funcmask}" --Ostats --nobet --mmthresh=0.5 --report --tr=${TR}
  ...
  do
    ...
    cluster -i ${file} -t 1.96 -d ${dlh_number} --volume=${funcmask_numvoxels} -p 0.05 --minclustersize --osize="${clusterfol}/sizeimage_${j}.nii.gz" --no_table > ${clusterfol}/curr_cluster.txt
  ...
  fslmaths ${melfol}/ICclusterthresh_zstat.nii.gz -abs -mul "${ROIcalcfol}/highprobmask.nii.gz" "${ROIcalcfol}/fullvolICA_adj.nii.gz"
  # calculate without clusterizing
  fslmaths ${melfol}/ICthresh_zstat.nii.gz -abs -thr 1.96 -mul "${ROIcalcfol}/highprobmask.nii.gz" "${ROIcalcfol}/fullvolICA_noclustering.nii.gz"
  ...
  # GM voxels
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${GMmask}" "${ROIcalcfol}/GMICA_weighted.nii.gz"
  ...
  # WM voxels
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${WMmask}" "${ROIcalcfol}/WMICA_weighted.nii.gz"
  ...
  # CSF voxels
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${CSFmask}" "${ROIcalcfol}/CSFICA_weighted.nii.gz"
  ...
  # Edge voxels
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${Edgemask}" "${ROIcalcfol}/EdgeICA_weighted.nii.gz"
  ...
  # Outbrain voxels
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${Outbrainmask}" "${ROIcalcfol}/OutbrainICA_weighted.nii.gz"
  ...
  # Inbrain voxels
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${Inbrainmask}" "${ROIcalcfol}/InbrainICA_weighted.nii.gz"
  ...
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${WMCSFmask}" "${ROIcalcfol}/WMCSFICA_weighted.nii.gz"
  ...
  fslmaths "${ROIcalcfol}/fullvolICA_adj.nii.gz" -mul "${Susceptmask}" "${ROIcalcfol}/SusceptICA_weighted.nii.gz"
