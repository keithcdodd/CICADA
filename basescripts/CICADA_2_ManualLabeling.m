function [cleaned_file] = CICADA_2_ManualLabeling(output_dir, IC_manual_checker)
% 
...
funcfile_niftiinfo = niftiinfo(funcfile);


tr = funcfile_niftiinfo.PixelDimensions(4);
numvolumes = funcfile_niftiinfo.ImageSize(4);
repetitiontime = tr; TR = tr;
...
load([output_dir, '/ic_auto_selection/DecisionVariables_Auto.mat'])
...
[B_manual, index_manual] = sortrows(temp_manual_table, 'PotentialICs', 'ascend'); % put in order from IC 1:end
[B_auto, index_auto] = sortrows(temp_auto_table, 'PotentialICs', 'ascend'); % put in order from IC 1:end
...
signal_indices = Results_Auto.signal_indices;
...
%% TO DO: GO FROM HERE 03/06/2024 Wednesday 4:30PM
...
potential_signal_ICs = IC_checker_table.PotentialICs;
...
writetable(Results.compare_cleaning, 'compare_manual_cleaning.csv')
...
save('DecisionVariables_Manual.mat', 'cadicafol', 'subject_id', 'session_id', ...
    'taskname', 'ICs', 'Data', 'Tables', 'Results_Auto', 'Results')
